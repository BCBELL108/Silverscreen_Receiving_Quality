...
def get_engine():
    ...
    # Try to use Streamlit secrets first (recommended for production)
    try:
        db_url = st.secrets["db_url"]
    except Exception:
        # Fallback to your Neon connection string (useful for local dev / emergency)
        # If you later set secrets["db_url"], this fallback will be ignored.
        db_url = (
            "postgresql://neondb_owner:"
            "npg_3TYnOM1Cqser"
            "@ep-steep-mountain-ah1dmpil-pooler.c-3.us-east-1.aws.neon.tech"
...
def main():
    ...
    try:
        ...
        st.error("Could not connect to database. Check secrets['db_url'] or the fallback Neon URL.")
        st.exception(e)
        return
